...
import { getUserAgents, deleteAgent } from "../../../utils/question";
import DashboardHeader from "../dashboardheader";
import Pagination from "@/app/pagination";

function AudioQuestions() {
  ...
  useEffect(() => {
    if (userData) {
      ...
      getUserAgents(users[1].userId, token).then((res) => {
        // console.log(res)
        if (res.length === 0) {
          setQuestions([]);
          setLoading(false);
  ...
  const handleDelete = (id: string) => {
    ...
    deleteAgent(id, userToken).then(() => {
      setRemove((remove) => !remove);
    })
  };
  const handleInvite = (id: string) => {
    navigator.clipboard.writeText(
      `${baseURL}/base/audio-video/${id}?id=${userData.userId}`,
    );
  ...
  const handleUpdate = (id: string) => {
    ...
    router.push(
      {
        pathname: `/dashboard/questions/audio/${id}`,
        query: queryParams,
      },
      `/dashboard/questions/audio/${id}`,
    );
  };
      ...
      <Sidebar>
        {!loading && questions && (
          <div className="my-4 bg-white p-4 sm:ml-60 lg:px-12">
            <DashboardHeader
              imagePath="/audio-question.svg"
              heading="Audio Questions"
              paragraph="Manage Edit & share your audio questions."
            />
              ...
              <table className="w-full text-left font-work_sans text-sm text-gray-500 dark:text-gray-400">
                <thead className="rounded-2xl bg-table_header text-xs font-normal capitalize text-gray-700 dark:bg-gray-700 dark:text-gray-400">
                  <tr>
                    ...
                    <th
                      scope="col"
                      className="min-w-[250px] px-6 py-3 font-medium"
                    >
                      Prompt
                    </th>
                    <th scope="col" className="px-6 py-3 font-medium">
                    ...
                    {currentData && currentData.map((item: any) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-2.5">
                          {`${item.prompt.substring(0, 28)}${item.prompt.length > 30 ? "..." : ""
                            }`}
                        </td>
                        <td className="whitespace-nowrap px-6 py-2.5">
                        ...
                        <td className="px-6 py-2.5">
                          {/* <button
                                                    ...
                                                    type="button" className="font-normal border border-table_border flex justify-center items-center gap-x-1 px-3 py-1 pr-5 rounded-lg min-w-[180px] h-8 bg-primary-50 hover:bg-primary-200 text-primary-900">
                        ...
                        <td className="px-6 py-2.5">
                          <button
                            ...
                            onClick={() => handleInvite(item.slug)}
                            className="border-grey-50 disabled:text-grey-900 flex h-8 min-w-[180px] items-center justify-center gap-x-1 whitespace-nowrap rounded-lg border px-3 py-1 pr-5 font-normal text-gray-900 hover:bg-gray-100 disabled:bg-gray-200 disabled:opacity-20"
                          >
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              width="16"
            ...
            <div className="flex justify-between mt-5">
              <p className="text-neutral-500">{`Page ${currentPage} of ${Math.ceil(questions.length / postsPerPage)}`}</p>
